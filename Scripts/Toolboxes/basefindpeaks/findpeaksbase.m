function [Ypk,Xpk,Wpk,Ppk] = findpeaksbase(Yin,varargin)
%FINDPEAKSBASE Find local peaks in data (Extracted from MATLAB built-ins)
%   PKS = FINDPEAKSBASE(Y) finds local peaks in the data vector Y. A local peak
%   is defined as a data sample which is either larger than the two
%   neighboring samples or is equal to Inf.
%
%   [PKS,LOCS]= FINDPEAKSBASE(Y) also returns the indices LOCS at which the
%   peaks occur.
%
%   [PKS,LOCS] = FINDPEAKSBASE(Y,X) specifies X as the location vector of data
...
%
%   [PKS,LOCS] = FINDPEAKSBASE(Y,Fs) specifies the sample rate, Fs, as a
%   positive scalar, where the first sample instant of Y corresponds to a
%   time of zero.
%
%   [...] = FINDPEAKSBASE(...,'MinPeakHeight',MPH) finds only those peaks that
%   are greater than the minimum peak height, MPH. MPH is a real-valued
%   scalar. The default value of MPH is -Inf.
%
%   [...] = FINDPEAKSBASE(...,'MinPeakProminence',MPP) finds peaks guaranteed
...
%
%   [...] = FINDPEAKSBASE(...,'Threshold',TH) finds peaks that are at least
...
%   TH is zero.
%
%   FINDPEAKSBASE(...,'WidthReference',WR) estimates the width of the peak as
...
%
%   [...] = FINDPEAKSBASE(...,'MinPeakWidth',MINW) finds peaks whose width is
%   at least MINW. The default value of MINW is zero.
%
%   [...] = FINDPEAKSBASE(...,'MaxPeakWidth',MAXW) finds peaks whose width is
%   at most MAXW. The default value of MAXW is Inf.
%
%   [...] = FINDPEAKSBASE(...,'MinPeakDistance',MPD) finds peaks separated by
...
%
%   [...] = FINDPEAKSBASE(...,'SortStr',DIR) specifies the direction of sorting
...
%
%   [...] = FINDPEAKSBASE(...,'NPeaks',NP) specifies the maximum number of peaks
...
%   'SortStr')
%
%   [PKS,LOCS,W] = FINDPEAKSBASE(...) returns the width, W, of each peak by
%   linear interpolation of the left- and right- intercept points to the
%   reference defined by 'WidthReference'.
%
%   [PKS,LOCS,W,P] = FINDPEAKSBASE(...) returns the prominence, P, of each
%   peak.
%
%   FINDPEAKSBASE(...) without output arguments plots the signal and the peak
%   values it finds
%
%   FINDPEAKSBASE(...,'Annotate',PLOTSTYLE) will annotate a plot of the
...
%   load sunspot.dat
%   FINDPEAKSBASE(sunspot(:,2),sunspot(:,1),'MinPeakDistance',6)
%   xlabel('Year');
...
%   load mtlb
%   findpeaksbase(mtlb,Fs,'MinPeakProminence',1)
%
%   % Example 3:
...
%   findpeaksbase(x,Fs,'MinPeakWidth',.5e-3,'MaxPeakWidth',1e-3, ...
%             'Annotate','extents')
%
